...
void loginStreakCalculation() {
	...
	std::cout << "64-77  | 4\n";
	std::cout << "78-92  | 5\n";
	std::cout << "How many days in a row do you plan to log into the game during the current season? (1-" << seasonDuration << "): ";
...
void easyTasksCalculation() {
	...
	std::cout << "How many easy tasks you plan to compleate? (0-" << loginStreak << "): ";
	easyTasks = inputCheck(0, loginStreak);
	if (easyTasks > 0) {
		points += easyTasks * 2;
		std::cout << "How much minutes on average do you spend on an easy task? (1-720): ";
		easyTaskTime = inputCheck(1, 720);
		easyTasksTotalTime = easyTasks * easyTaskTime;
		std::cout << "Points gain = " << easyTasks * 2 << ", points total = " << points << ", time spend = " << easyTasksTotalTime << " (" << easyTasksTotalTime / 60 << " hours)\n";
	}
	line();
}
...
void mediumTasksCalculation() {
	...
	if (easyTasks > 0) {
		std::cout << "How many medium tasks you plan to compleate? (0-" << easyTasks << "): ";
		mediumTasks = inputCheck(0, easyTasks);
		if (mediumTasks > 0) {
			points += mediumTasks * 3;
			std::cout << "How much minutes on average do you spend on a medium task? (1-720): ";
			mediumTaskTime = inputCheck(1, 720);
			mediumTasksTotalTime = mediumTasks * mediumTaskTime;
			std::cout << "Points gain = " << mediumTasks * 3 << ", points total = " << points << ", time spend = " << mediumTasksTotalTime << " (" << mediumTasksTotalTime / 60 << " hours)\n";
		}
	}
	else {
...
void specialTasksCalculation() {
	...
	if (mediumTasks > 0) {
		std::cout << "How many special tasks you plan to compleate? (0-" << mediumTasks << "): ";
		specialTasks = inputCheck(0, mediumTasks);
		if (specialTasks > 0) {
			points += specialTasks * 5;
			std::cout << "How many extra minutes (after previous tasks) will it take on average to complete a special task? (0-1440): ";
			specialTaskTime = inputCheck(0, 1440);
			specialTasksTotalTime = specialTasks * specialTaskTime;
			std::cout << "Points gain = " << specialTasks * 5 << ", points total = " << points << ", time spend = " << specialTasksTotalTime << " (" << specialTasksTotalTime / 60 << " hours)\n";
		}
	}
	else {
...
void challengesTasksCalculation() {
	...
	std::cout << "How many challenges you plan to compleate? (0-14): ";
	...
	if (challenges > 0) {
		...
		std::cout << "How many extra minutes (after previous tasks) will it take on average to complete a challenge? (0-1440): ";
		challengeTime = inputCheck(0, 1440);
		challengesTotalTime = challenges * challengeTime;
		std::cout << "Points gain = " << challengesPoints << ", points total = " << points << ", time spend = " << challengesTotalTime << " (" << challengesTotalTime / 60 << " hours)\n";
	}
	line();
}
...
void battlePassCheck() {
	...
	std::cout << "Are you going to buy an Battle Pass? (0 - No, 1 - Yes, 2 - Improved Battle Pass): ";
...
void levelsPurchase () {
	...
	if (battlePass == 0) {
		...
		std::cout << "How many levels are you planning to buy? (0-200): ";
...
void results () {
	...
	std::cout << std::endl;
	std::cout << "Sorce                | Minutes spend\n";
	std::cout << "Easy tasks           | " << easyTasksTotalTime << std::endl;
	std::cout << "Medium tasks         | " << mediumTasksTotalTime << std::endl;
	std::cout << "Special tasks        | " << specialTasksTotalTime << std::endl;
	std::cout << "Challenges           | " << challengesTotalTime << std::endl;
	std::cout << "TOTAL TIME = " << (easyTasksTotalTime + mediumTasksTotalTime + specialTasksTotalTime + challengesTotalTime) / 60 << " hours " << (easyTaskTime + mediumTaskTime + specialTaskTime + challengeTime) % 60 << " minutes (" << ((easyTasksTotalTime + mediumTasksTotalTime + specialTasksTotalTime + challengesTotalTime) / 60) / loginStreak  << " hours per day)\n";
	std::cout << std::endl;
	std::cout << "Sorce                | Golden Eagles spend\n";
	std::cout << "Battle Pass          | " << goldenEaglesSpendOnBattlePass << std::endl;
	std::cout << "Purchased levels     | " << goldenEaglesSpendOnLevels << std::endl;
	std::cout << "TOTAL GOLDEN EAGLES SPEND = " << goldenEaglesSpendOnLevels + goldenEaglesSpendOnBattlePass << std::endl;
	line();
}
